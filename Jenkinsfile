node {
  ...
  sh "mkdir dist -p"
//  sh "cp -r {.dockerignore,docker-compose.yml,Dockerfile,server} dist && cd dist && tar cvzf weather-app-project_current.tar.gz *"
  sh "cp -r {.dockerignore,docker-compose.yml,Dockerfile,server} dist && cd dist && tar --ignore-failed-read cvzf weather-app-project_current.tar.gz *"
